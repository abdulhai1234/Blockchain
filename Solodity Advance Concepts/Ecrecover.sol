...
// to extract the address of the sender based on the digital signature.
contract AddressValidations {
    ...
    function ExtractAddressAnotherVersion(bytes32 hash, bytes memory sig) public pure returns(address) {
        if (sig.length != 65) {
            return address(0);
        }
        bytes32 rValue;
